...
def main():
    ...
    sents_file_name = os.listdir(args.sents_dir_name)[0]
    deep_ud_file_name = os.listdir(args.deep_ud_dir_name)[0]
    ...
    with open(sents_file_name) as story_sentences:
        ...
        story_context_sents = []
        for sent, deep_ud in story_sentences, deep_uds:
            sent = sent.strip()
        ...
        while num_stories < 30:
            ...
            if match_staart.match(dev_content_selection_src[-1]):
                num_stories += 1
        num_stories = 0
        while num_stories < 30:
            ...
            if match_staart.match(dev_surface_realization_src[-1]):
                num_stories += 1

        content_selection_dir_name = \
            os.mkdir(os.path.join(args.output_dir_name, 'content_selection'))
        surface_realization_dir_name = \
            os.mkdir(os.path.join(args.output_dir_name, 'surface_realization'))
        input_file_root = \
